...
from market_data_source import build_market_data_source
...
def _sync_symbol_timeframe(store: MarketDataStore, source, symbol_id: int, symbol: str, timeframe: str, limit: int) -> int:
    checkpoint = store.get_checkpoint(symbol_id, timeframe)
    candles = source.get_candles(symbol, timeframe, since=checkpoint, limit=limit)
    if not candles:
        return 0
...
def run(
    ...
    source = build_market_data_source(config.market_data_source)
    ...
    try:
        symbol_ids = {
            symbol: store.ensure_symbol(
                canonical_symbol=symbol, broker_symbol=symbol, source=config.market_data_source
            )
        ...
        for symbol in config.market_symbols:
            for timeframe in config.market_timeframes:
                count = _sync_symbol_timeframe(
                    store, source, symbol_ids[symbol], symbol, timeframe, INITIAL_BACKFILL_LIMIT
                )
        ...
        while True:
            ...
            for symbol in config.market_symbols:
                for timeframe in config.market_timeframes:
                    count = _sync_symbol_timeframe(
                        store, source, symbol_ids[symbol], symbol, timeframe, POLL_BATCH_LIMIT
                    )
                    if count:
