...
def _build_client(config) -> Mt5Client:
    if config.mock_mode:
        logger.info("MOCK_MODE attivo: nessuna dipendenza da Wine/MT5, uso MockMt5Client.")
...
def run(env: Optional[dict] = None) -> int:
    ...
    logger.info(
        "Avvio mt5-cloud-worker: mock_mode=%s dry_run=%s poll_interval=%ss api_target_configurato=%s",
        config.mock_mode,
        config.dry_run,
    ...
    snapshot_store = SnapshotStore(file_path=None if config.mock_mode else SNAPSHOT_FILE_REAL_MODE)
    sender = EventSender(
    ...
    try:
        while True:
            ...
            if config.mock_mode and getattr(client, "is_scenario_finished", False) and not scenario_completion_logged:
                logger.info(
