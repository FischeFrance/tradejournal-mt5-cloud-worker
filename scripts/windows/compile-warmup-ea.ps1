...
$source = Join-Path $repo 'mt5\experts\TradeJournalWarmup.mq5'
$stage = 'C:\TradeJournal\artifacts\mql5'
$target = Join-Path $stage 'TradeJournalWarmup.mq5'
$binary = [IO.Path]::ChangeExtension($target, '.ex5')
$log = 'C:\TradeJournal\logs\TradeJournalWarmup-compile.log'
$result = 'C:\TradeJournal\logs\TradeJournalWarmup-compile-result.json'

if (-not (Test-Path $editor)) { throw 'MetaEditor64.exe not found.' }
if (-not (Test-Path $source)) { throw 'TradeJournalWarmup.mq5 not found.' }
...
if (-not (Test-Path $binary) -or -not (Select-String -Path $log -SimpleMatch 'Result: 0 errors, 0 warnings' -Quiet -ErrorAction SilentlyContinue)) {
    throw "Warm-up EA compilation failed; inspect $log"
}
