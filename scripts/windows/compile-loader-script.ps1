...
$source = Join-Path $repo 'mt5\experts\TradeJournalLoader.mq5'
$stage = 'C:\TradeJournal\artifacts\mql5'
$target = Join-Path $stage 'TradeJournalLoader.mq5'
$binary = [IO.Path]::ChangeExtension($target, '.ex5')
$log = 'C:\TradeJournal\logs\TradeJournalLoader-compile.log'
$result = 'C:\TradeJournal\logs\TradeJournalLoader-compile-result.json'

if (-not (Test-Path $editor)) { throw 'MetaEditor64.exe not found.' }
if (-not (Test-Path $source)) { throw 'TradeJournalLoader.mq5 not found.' }
...
if (-not (Test-Path $binary) -or -not (Select-String -Path $log -SimpleMatch 'Result: 0 errors, 0 warnings' -Quiet -ErrorAction SilentlyContinue)) {
    throw "Loader script compilation failed; inspect $log"
}
