...
def _validate_history_batch(response: dict, expected_count: int) -> dict:
    if response.get("error_code") == "lease_lost":
        if set(response) != {"api_version", "error_code"}:
            raise AgentContractError(
                "history batch lease-loss response does not match contract V1"
            )
        return response
    if set(response) != {
        "api_version", "accepted", "inserted", "duplicates"
    } or response.get("api_version") != "1":
        raise AgentContractError("history batch response does not match contract V1")
    for field in ("accepted", "inserted", "duplicates"):
        value = response[field]
        if not isinstance(value, int) or isinstance(value, bool) or value < 0:
            raise AgentContractError("history batch counts are invalid")
    if (
        response["accepted"] != expected_count
        or response["inserted"] + response["duplicates"] != expected_count
    ):
        raise AgentContractError("history batch acknowledgement count mismatch")
    return response


def _validate_history_file_prepare(response: dict) -> dict:
    if response.get("error_code") == "lease_lost":
        if set(response) != {"api_version", "error_code"}:
            raise AgentContractError("history upload lease-loss response is invalid")
        return response
    if set(response) != {
        "api_version", "already_imported", "object_path", "upload_url", "expires_in",
        "accepted", "inserted", "duplicates",
    } or response.get("api_version") != "1":
        raise AgentContractError("history upload response does not match contract V1")
    path = response.get("object_path")
    if not isinstance(path, str) or not re.fullmatch(
        r"[0-9a-f-]{36}/[0-9a-f-]{36}/[0-9a-f-]{36}/history\.json\.gz", path
    ):
        raise AgentContractError("history object path is invalid")
    imported = response.get("already_imported")
    if not isinstance(imported, bool):
        raise AgentContractError("history imported state is invalid")
    if imported:
        if response.get("upload_url") is not None or response.get("expires_in") != 0:
            raise AgentContractError("imported history upload response is invalid")
    elif (
        not isinstance(response.get("upload_url"), str)
        or not response["upload_url"].startswith("https://")
        or response.get("expires_in") != 7200
    ):
        raise AgentContractError("history signed upload is invalid")
    for field in ("accepted", "inserted", "duplicates"):
        value = response.get(field)
        if not isinstance(value, int) or isinstance(value, bool) or value < 0:
            raise AgentContractError("history upload counts are invalid")
    if response["inserted"] + response["duplicates"] != response["accepted"]:
        raise AgentContractError("history upload counts do not reconcile")
    return response


def _validate_history_file_import(response: dict, expected_count: int) -> dict:
    if response.get("error_code") == "lease_lost":
        if set(response) != {"api_version", "error_code"}:
            raise AgentContractError("history import lease-loss response is invalid")
        return response
    if set(response) != {
    ...
    for field in ("accepted", "inserted", "duplicates"):
        value = response[field]
        if not isinstance(value, int) or isinstance(value, bool) or value < 0:
            raise AgentContractError("history import counts are invalid")
    if (
        response["accepted"] != expected_count
        or response["inserted"] + response["duplicates"] != expected_count
        or not isinstance(response["object_deleted"], bool)
    ):
        raise AgentContractError("history import acknowledgement mismatch")
    return response
...
class AgentApiClient:
    ...
    def session(self) -> dict:
        return self.request("POST", "session", {"api_version": self.API_VERSION})
    ...
    def history_batch(
        ...
        if not 1 <= len(events) <= 50:
            raise ValueError("history batch must contain between 1 and 50 events")
        return _validate_history_batch(
            self.request(
                "POST",
    ...
    def history_file_prepare(
        ...
        *,
        ...
        return _validate_history_file_prepare(
            self.request(
                "POST",
                f"jobs/{job_id}/history-upload",
                {
                    "api_version": self.API_VERSION,
                    "lease_id": lease_id,
    ...
    def upload_history_file(self, signed_url: str, payload: bytes) -> None:
        parsed = urlparse(signed_url)
        if (
            (parsed.scheme, parsed.hostname, parsed.port) != self._origin
            or not parsed.path.startswith(
                "/storage/v1/object/upload/sign/mt5-history-imports/"
            )
            or not parsed.query
        ):
            raise AgentContractError("history signed upload target is invalid")
        for attempt in range(1, self._max_retries + 2):
            try:
                ...
                continue
            if response.is_redirect:
                raise RuntimeError("signed upload redirect refused")
            if response.status_code >= 500 and attempt <= self._max_retries:
                self._sleep(self._backoff_delay(attempt))
                continue
            response.raise_for_status()
    ...
    def history_file_import(
        ...
        return _validate_history_file_import(
            self.request(
                "POST",
