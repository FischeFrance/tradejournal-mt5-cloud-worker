...
class NativeMt5Runtime:
    ...
    def _complete_live_update(self, config: Path) -> None:
        ...
        try:
            ...
            WindowsSecretStore.restrict_acl(staging)
            ...
            for source in source_directory.iterdir():
                ...
                if (
                    ...
                    and re.fullmatch(r"[A-Za-z0-9_-]+\.[0-9]{4,6}", source.name)
                ):
                    payload_sources.append(source)
            payload_sources.sort(key=lambda value: value.name.casefold())
            payload_digests = {
                source: self._sha256(source) for source in payload_sources
            }
            for source in payload_sources:
                destination = staging / source.name
                shutil.copy2(source, destination)
                if self._sha256(destination) != payload_digests[source]:
                    raise NativeMt5Error("mt5_update_copy_integrity_failed")
            (staging / "temp").mkdir()
            staged_updater = staging / candidate.executable.name
            updater_signer = self._verify_metaquotes_signature(staged_updater)
            ...
            # Re-check immediately before deleting the exact cache files that
            # were copied.  Removing them prevents the next MT5 start from
            # relaunching an already-consumed LiveUpdate.  Any unrelated file
            # in the user profile is deliberately left untouched.
            for source, digest in payload_digests.items():
            ...
            if self._verified_vendor_update_callback is not None:
                try:
                    ...
                    # The isolated instance is already updated and sealed. A
                    # golden-template maintenance failure must not turn a valid
                    # account login into a failed connection; it is retried on
                    # the next verified vendor update and logged for operators.
                    logger.exception("verified MT5 template promotion failed")
    ...
    def _start_and_wait_for_authorization(
        ...
        # A downloaded cache file has no trustworthy binding to one of the
        # isolated terminals. Wait until MetaQuotes launches it with the exact
        # /path and /config arguments, then _wait_for_authorization can bind and
        # complete only that instance's update.
        for attempt in range(2):
            checkpoint = self._journal_checkpoint()
            self._start_process(config)
            monitor = self._start_auth_failure_monitor(timeout)
            try:
                ...
                if str(exc) != "mt5_live_update_required" or attempt != 0:
                    raise
                self._complete_live_update(config)
            finally:
    ...
    def _wait_for_authorization(
        ...
        while time.monotonic() < deadline:
            ...
            for line in lines:
                ...
                if "\tliveupdate\tstart " in folded and " /update " in folded:
            ...
            if update_config is not None:
                ...
                if update_announced:
                    ...
                    # polling until the exact updater appears or timeout wins.
                    time.sleep(0.1)
                    continue
