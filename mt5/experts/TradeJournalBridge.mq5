...
//+------------------------------------------------------------------------+
//| Handshake leggero al bootstrap. Lo storico puo' richiedere piu' tempo  |
//| del timeout di provisioning: l'Agent deve quindi poter validare subito |
//| identita', connessione e accesso in sola lettura prima del backfill.    |
//+------------------------------------------------------------------------+
void WriteBootstrapHandshake()
  {
   g_event_seq++;
   long sequence = g_event_seq;
   WriteJsonAtomic("account.json", BuildEnvelope(BuildAccountJson(), sequence));
...
int OnInit()
  {
   ...
   // Non serializzare storico o candele prima del handshake: il worker puo'
   // cosi' concludere il provisioning anche per account con molti dati.
   WriteBootstrapHandshake();
